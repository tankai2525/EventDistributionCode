/* dispatchTouchEvent：9262 ，onTouchEvent：10268
...
public class View implements Drawable.Callback, KeyEvent.Callback,
    ...
    */
     ...
     * view if it is the target.//如果当前view是目标view, 就会传入点击事件到目标view
     *
     * @param event The motion event to be dispatched.//分发的点击事件，包含点击动作 点击时间 点击位置 等等
     * @return True if the event was handled by the view, false otherwise.//如果事件被处理了返回true，否则返回false
     */
    public boolean dispatchTouchEvent(MotionEvent event) {
		...
		//返回正在执行的掩码操作，无指针索引信息。使用getactionindex()返回指针行为相关的指标。
        ...
        if (onFilterTouchEventForSecurity(event)) {//过滤触摸事件以应用安全策略
            //noinspection SimplifiableIfStatement//无检查简化if语句
            ListenerInfo li = mListenerInfo;
            if (li != null && li.mOnTouchListener != null
                    && (mViewFlags & ENABLED_MASK) == ENABLED
                    && li.mOnTouchListener.onTouch(this, event)) {
				//关键代码，如果view设置了OnTouchListener监听并且是enabled状态，就会调用OnTouchListener的onTouch方法
                result = true;
            }

            if (!result && onTouchEvent(event)) {
				//关键代码，如果onTouch返回false, onTouchEvent方法会被调用，否则不会调用
				result = true;
				//如果onTouch返回true那就代表view处理了改事件
				//如果onTouch返回false那就代表view没有处理，交给父控件去处理，父控件就会执行自己的父类的dispatchTouchEvent，就相当于把父类当成view回到了这段代码
            }
        }
    ...
    /**
     * Implement this method to handle touch screen motion events.//用这个方法来处理点击事件
     * <p>
     * If this method is used to detect click actions, it is recommended that
     * the actions be performed by implementing and calling
     * {@link #performClick()}. This will ensure consistent system behavior,
	 * 如果这个方法被用于检测单击操作，建议实现和调用performclick()。这将确保一致的系统行为，包括：
     * including:
     * <ul>
     * <li>obeying click sound preferences//dispatching OnClickListener calls
     * <li>dispatching OnClickListener calls//事件分发中调用OnClickListener的方法
     * <li>handling {@link AccessibilityNodeInfo#ACTION_CLICK ACTION_CLICK} when
     * accessibility features are enabled//处理action_click辅助功能时启用
     * </ul>
     *
     * @param event The motion event.
     * @return True if the event was handled, false otherwise.
    ...
    public boolean onTouchEvent(MotionEvent event) {
        final float x = event.getX();//获取点击位置到view最左边距离
        final float y = event.getY();//获取点击位置到view最上边距离
        final int viewFlags = mViewFlags;
        final int action = event.getAction();

        if ((viewFlags & ENABLED_MASK) == DISABLED) {//如果view是disable状态， 那也就是说disable的view也能进入到onTouchEvent方法
            ...
            return (((viewFlags & CLICKABLE) == CLICKABLE
                    || (viewFlags & LONG_CLICKABLE) == LONG_CLICKABLE)
                    || (viewFlags & CONTEXT_CLICKABLE) == CONTEXT_CLICKABLE);
			//如果view是disable状态，返回view的clickable,long_clickable,context_clickable三种状态的逻辑或结果
			//如果view是disable状态，view的clickable,long_clickable,context_clickable三种状态都是false, 事件就会交给父控件处理，否则会消费掉
        }
        ...
        if (((viewFlags & CLICKABLE) == CLICKABLE ||
                (viewFlags & LONG_CLICKABLE) == LONG_CLICKABLE) ||
                (viewFlags & CONTEXT_CLICKABLE) == CONTEXT_CLICKABLE) {//有一个为true就会处理这个事件
            switch (action) {
